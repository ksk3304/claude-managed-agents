...
describe('makoto-prime spec bundle (generated src/data/*.ts)', () => {
  ...
  it('self-introspection instructions are bundled with current skill/tool boundaries', async () => {
    ...
    expect(r.systemPrompt).toContain('{"topic": "workspace", "include_sources": true}');
    expect(r.systemPrompt).not.toContain('{"detail": "all", "include_sources": true}');
    expect(r.systemPrompt).toContain('Cloud Run 版 / Cloudflare 版 / 設計 / 未確認');
    expect(r.systemPrompt).toContain('`custom_tools` と `action_markers`');
