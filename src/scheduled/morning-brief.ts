...
export const MORNING_AI_NEWS_SETO_CRON = '20 20 * * *';
export const MORNING_AI_NEWS_SETO_JOB_ID = 'morning_ai_news_seto_dm';

export const MORNING_BRIEF_SETO_PROMPT = `瀬戸さん向けの朝ブリーフを作成するタスク。情報収集（memory / issue 一覧 / session-log 等の参照）はツールで自由に行ってよいが、テキストとして文章を出力するのは最終ブリーフ 1 回のみとする。
...
- ===BRIEF_FINAL=== の行より後に、ブリーフ本文（下記 4 セクション）だけを 1 回だけ書く。
...
export const MORNING_AI_NEWS_SETO_PROMPT = `AI関連の最新ニュースを毎朝3本だけ届けるタスク。Webで確認し、出典URLを必ず付ける。

# 出力規約（最優先・絶対厳守）
- あなたが出力する最初の可視テキスト行は、必ず次の 1 行だけにする: ===BRIEF_FINAL===
- \`===BRIEF_FINAL===\` は出力全体で 1 回だけ書く。
- それ以前にテキスト（思考・前置き・進捗・「調べます」等）を一切書かない。
- ===BRIEF_FINAL=== の行より後に、ニュース本文だけを 1 回だけ書く。
- 本文を 1200 字以内に収める。
- ニュースは必ず 3 件。各件にタイトル、概要、出典URLを含める。
- 出典URLが確認できない話題は採用しない。
- 直近1週間以内の情報を優先し、大手モデル更新 / エージェント実装 / 規制・政策 / プロダクトリリースから重要度の高いものを選ぶ。
- 古い一般論、出典なし要約、内部状態説明を書かない。

# 本文形式
1. タイトル
   概要: 2〜3行で何が起きたか、なぜ重要か。
   出典: URL
...
# 取得できない場合
- 3件揃わない場合は、揃った件数だけ出すのではなく「取得失敗: 出典URL付きで3件確認できませんでした」とだけ書く。
- 失敗理由にツール名、API名、環境名、attach、session、内部エラー等を書かない。`;
...
export async function enqueueMorningBriefSeto(
  ...
  const eventKey = `scheduled:${MORNING_BRIEF_SETO_JOB_ID}:${dateLabel}:${nowMs}`;
  const owner = newClaimOwner('cron-morning-brief-seto');
  ...
  const payload = buildMorningBriefChatEvent(nowMs, eventKey);
  ...
  await recordRuntimeEvent(env, {
    ...
    eventType: 'scheduled_morning_brief_enqueue_start',
    source: 'cron.morning-brief',
    detail: { job_id: MORNING_BRIEF_SETO_JOB_ID, date_label: dateLabel },
  });
  ...
  try {
    ...
    await recordRuntimeEvent(env, {
      eventKey,
      messageId: payload.message?.name,
      eventType: 'scheduled_morning_brief_enqueue_failed',
      level: 'error',
      source: 'cron.morning-brief',
  ...
  await recordRuntimeEvent(env, {
    eventKey,
    messageId: payload.message?.name,
    eventType: 'scheduled_morning_brief_enqueued',
    source: 'cron.morning-brief',
    detail: {
      job_id: MORNING_BRIEF_SETO_JOB_ID,
...
export async function enqueueMorningAiNewsSeto(
  env: Env,
  nowMs: number = Date.now(),
): Promise<MorningBriefEnqueueResult> {
  const dateLabel = jstDateLabel(nowMs);
  const eventKey = `scheduled:${MORNING_AI_NEWS_SETO_JOB_ID}:${dateLabel}:${nowMs}`;
  const owner = newClaimOwner('cron-morning-ai-news-seto');
  const claim = await tryClaim(env.DB, eventKey, owner);
  if (claim.state === 'DONE_DUPLICATE') return { kind: 'duplicate', eventKey };
  if (claim.state === 'LEASE_ALIVE') return { kind: 'lease_alive', eventKey };
  if (claim.owner === undefined || claim.version === undefined) {
  ...
  const payload = buildMorningAiNewsChatEvent(nowMs, eventKey);
  const queueMsg: ChatQueueMessage = {
    eventKey,
    receivedAtMs: nowMs,
    claim: { owner: claim.owner, version: claim.version },
  ...
  await recordRuntimeEvent(env, {
    eventKey,
    messageId: payload.message?.name,
    eventType: 'scheduled_morning_ai_news_enqueue_start',
    source: 'cron.morning-ai-news',
    detail: { job_id: MORNING_AI_NEWS_SETO_JOB_ID, date_label: dateLabel },
  });
  ...
  try {
    ...
    await recordRuntimeEvent(env, {
      eventKey,
      messageId: payload.message?.name,
      eventType: 'scheduled_morning_ai_news_enqueue_failed',
      level: 'error',
      source: 'cron.morning-ai-news',
      detail: { error: error instanceof Error ? error.message : String(error) },
    });
  ...
  await recordRuntimeEvent(env, {
    eventKey,
    messageId: payload.message?.name,
    eventType: 'scheduled_morning_ai_news_enqueued',
    source: 'cron.morning-ai-news',
    detail: {
      job_id: MORNING_AI_NEWS_SETO_JOB_ID,
      date_label: dateLabel,
      text_chars: payload.message?.text?.length ?? 0,
    },
  });
...
export function buildMorningBriefChatEvent(
  ...
  return {
    ...
    message: {
      ...
      text: `${todayPrefix(nowMs)}${MORNING_BRIEF_SETO_PROMPT}`,
      annotations: [],
      attachment: [],
    },
  };
...
export function buildMorningAiNewsChatEvent(
  nowMs: number,
  eventKey: string,
): ChatEventPayload {
  const messageName = `${MORNING_BRIEF_SETO_SPACE}/messages/${safeMessageId(eventKey)}`;
  return {
    ...
    user: {
      name: 'users/scheduled-morning-ai-news-seto',
      displayName: 'MAKOTO Scheduler',
      email: MORNING_BRIEF_SETO_EMAIL,
    },
    message: {
      ...
      text: `${todayPrefix(nowMs)}${MORNING_AI_NEWS_SETO_PROMPT}`,
      annotations: [],
      attachment: [],
    },
  };
