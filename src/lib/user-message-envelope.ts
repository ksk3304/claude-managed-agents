...
export const MAIL_INTENT_INSTRUCTIONS =
  ...
  'このターンはメール送信意図として扱う。\n' +
  '- 宛先・件名・本文が揃う、または宛先と明確な内容名がある場合は、送信前確認を挟まず EMAIL_SEND マーカーを出す。\n' +
  '- 「こんにちはメール」「お礼メール」「確認メール」などの短い内容名は、件名と本文の素材として使う。「こんにちはメール」は件名「こんにちは」、本文「こんにちは」で足りる。\n' +
  '- `to` は回答・レビュー・返信を求める直接対象1名だけ。AとBに聞く/レビュー依頼する場合は、A宛とB宛の EMAIL_SEND マーカーを別々に出す。\n' +
  '- `cc` / `bcc` は共有・監査・同報対象だけ。複数人が named されても、回答対象を `cc` に回さない。「AとBに聞いて。全てCCにC」は A宛cc=C と B宛cc=C の2通。\n' +
